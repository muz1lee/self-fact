...
class RLHFArguments:
    ...
    pref_loss: Literal["sigmoid", "hinge", "ipo", "kto_pair", "orpo", "simpo", "distill"] = field(
        default="sigmoid",
    ...
    self_refine_threshold: Optional[int] = field(
        default=1,
        metadata={"help": "SPA's self_refile_threshold"}
        )
    
    refine_stragth: Optional[float] = field(
        default=0.1,
        metadata={"help": "SPA's self_refine_stragth"}
        )
    
    decouple_denoising: Optional[float] = field(
        default=0.5,
        metadata={"help": "SPA's decouple_denoising"}
        )

    confidence_dir: Optional[str] = field(
        default="save_confidence",
        metadata={"help": "save point of reward from response"}
        )
    
    ref_model_for_refine: Optional[str] = field(
        default="Meta-Llama-3-8B-Instruct",
        metadata={"help": "reference model for self labeling"}
    )

    save_confidence_name: Optional[str] = field(
        default=".json",
        metadata={"help": "save_confidence_name"}
    )
    
    get_confidence: Optional[bool] = field(
        default=False,
        metadata={"help": "get_confidence"}
    )

    confidence_type: Optional[str] = field(
        default="policy", # policy or ref
