...
def vllm_infer(
    ...
    with open(save_name, "w", encoding="utf-8") as f:
        for idx, (prompt, result, label) in enumerate(zip(prompts, results, labels)):
            answers = []
            ...
            if len(answers) >= 2:
                ...
                processed_data.append({
                    ...
                    'rejected': [
                        {"content": prompt, "role": "user"},
                        {"content": answers[1], "role": "assistant"}
                    ]
                })
    ...
    for item in processed_data:
        preference_json.append({
            "instruction": item['prompt'],  # 使用prompt作为instruction
            "input": "",  # 这里设为空字符串，因为我们没有额外的输入
            "chosen": item['chosen'][1]['content'],  # 取第一个回答作为优质回答
            "rejected": item['rejected'][1]['content']  # 取第二个回答作为劣质回答
        })
    
    # 保存为JSON文件
    json_output_path = save_name.replace('.jsonl', '_preference.json')
    with open(json_output_path, 'w', encoding='utf-8') as f:
        json.dump(preference_json, f, ensure_ascii=False, indent=2)

    print("*" * 70)
    print(f"{len(prompts)} generated results have been saved at {save_name}")
    print(f"Preference JSON with {len(preference_json)} pairs has been saved at {json_output_path}")
    print("*" * 70)
